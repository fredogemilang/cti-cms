...
**CDT runs WITHOUT critical CSS since 2026-09-22.** With the gate gone, the curated 16KB critical CSS (mobile-oriented) left the desktop header unstyled until the async sheet arrived (FOUC: giant chevrons, expanded mega-menus), and the swap itself started 344 CSS transitions at once + a second full layout. Every other page already loads the stylesheet render-blocking (Homepage Only was on), so the homepage now does the same: `pageopt_critical_css` and `pageopt_deferred_stylesheets` are empty on production (backup of the old values: `/home/cdtadmin/pageopt-backup-20260922.json`). Result: mobile 94–99 / desktop 100, CLS 0, no FOUC possible. If someone re-enables critical CSS, it must cover the fold at desktop AND mobile widths, and the deferred mode now stamps `html.css-pending` (transitions suppressed until the last deferred sheet's `onload`) — see `OptimizeHtml::deferStylesheets`.
