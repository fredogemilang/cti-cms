...
<div class="max-w-3xl mx-auto space-y-6">
    ...
    <x-admin.ui.card padding="p-8">
        <form action="{{ route('admin.menus.update', $menu) }}" method="POST" class="space-y-6">
            ...
            <!-- Title -->
            <x-admin.ui.input name="title" label="Menu Title" :value="old('title', $menu->title)" required />
            ...
            <!-- Icon -->
            <div>
                <x-admin.ui.input name="icon" label="Icon Class (Optional)" :value="old('icon', $menu->icon)" placeholder="e.g., fas fa-home, heroicon-o-home" />
                <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">Font Awesome or Heroicon class name</p>
            </div>

            <!-- Route -->
            <x-admin.ui.input name="route" label="Route Name (Optional)" :value="old('route', $menu->route)" placeholder="e.g., admin.users.index" />
            ...
            <!-- Order -->
            <div>
                <x-admin.ui.input type="number" name="order" label="Display Order" :value="old('order', $menu->order)" required min="0" />
                <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">Lower numbers appear first</p>
            </div>

            <!-- Is Active -->
            <x-admin.ui.checkbox name="is_active" label="Active" :checked="old('is_active', $menu->is_active)" description="Show this menu item in navigation" />
