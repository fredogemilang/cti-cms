            ...
            <div class="p-6 space-y-6">
                ...
                <div class="rounded-2xl bg-white dark:bg-[#1A1A1A] border border-gray-200 dark:border-[#272B30] p-5 shadow-sm dark:shadow-none">
                    ...
                    <div class="flex flex-col gap-2">
                        <button type="button" @click="updateForm" :disabled="isSaving"
                            ...
                            <span x-show="!isSaving">Update Form</span>
...
@push('scripts')
@php
    $preparedFields = $form->fields->map(function($f) {
        $optionsText = '';
        if ($f->options) {
            $optionsText = collect($f->options)->map(function($o) {
        ...
        return [
            ...
            'column_width' => $f->column_width ?? 'full',
            'advanced_settings' => $f->advanced_settings ?? [],
            'conditional_logic' => $f->conditional_logic ?? ['action' => 'show', 'match_type' => 'all', 'conditions' => []],
        ];
    })->toArray();
    
    // Prepare settings with defaults - use form's direct columns, not settings array
    ...
    $preparedConfirmations = $form->confirmations ?? [
        'type' => 'message',
        'message' => 'Thank you for your submission!',
        'redirect_url' => '',
    ];
    
    $preparedSpamProtection = $form->spam_protection ?? [
        'honeypot' => true,
        'captcha_provider' => 'none',
    ];
@endphp
<script>
    // Form data passed from server - using unescaped JSON to prevent HTML entity encoding
    const FORM_DATA = {!! json_encode([
        'id' => $form->id,
        'name' => $form->name,
        'slug' => $form->slug,
        ...
        'confirmations' => $preparedConfirmations,
        'spam_protection' => $preparedSpamProtection,
    ]) !!};
    ...
    function formBuilder() {
        return {
            ...
            isActive: FORM_DATA.is_active ? '1' : '0',
            ...
            fieldSearch: '',
            settingsTab: 'general',
            submitButtonText: FORM_DATA.submit_button_text,
            
            // Notifications settings
            notifications: FORM_DATA.notifications,
            
            // Confirmation settings
            confirmations: FORM_DATA.confirmations,
            
            // Spam protection
            spamProtection: FORM_DATA.spam_protection,
            ...
            async updateForm() {
                ...
                try {
                    // Process fields to format options
                    ...
                    const response = await fetch("{{ route('admin.forms.update', $form) }}", {
                        method: 'PUT',
                        headers: {
                    ...
                    if (result.success) {
                        window.dispatchEvent(new CustomEvent('notify', { 
                            detail: [{ type: 'success', message: 'Form updated successfully!' }] 
                        }));
                        setTimeout(() => {
