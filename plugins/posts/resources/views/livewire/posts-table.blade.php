<div>
             ...
             <div class="inline-flex w-fit items-center bg-gray-100/50 dark:bg-[#0B0B0B]/30 p-1 rounded-2xl ring-1 ring-gray-200 dark:ring-[#272B30]">
                @php
                    $statuses = [
                        ...
                        'archived' => ['label' => 'Archived', 'count' => $statusCounts['archived']],
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-gray-50/50 dark:bg-[#0B0B0B]/20 border-b border-gray-100 dark:border-[#272B30]">
                        ...
                        <th class="px-4 py-6 text-[11px] font-bold text-[#6F767E] uppercase tracking-widest">Categories</th>
                        <th class="px-4 py-6 text-[11px] font-bold text-[#6F767E] uppercase tracking-widest">Status</th>
                        <th class="px-4 py-6">
                            <button wire:click="sortBy('views_count')" class="flex items-center gap-1 text-[11px] font-bold text-[#6F767E] uppercase tracking-widest hover:text-[#2563EB] transition-colors">
                                Views
                ...
                <tbody class="divide-y divide-gray-50 dark:divide-[#272B30]/30 transition-opacity duration-200" wire:loading.class="opacity-50 pointer-events-none">
                    ...
                    @php
                        $statusClasses = [
                            ...
                            'archived' => 'bg-amber-100 dark:bg-amber-900/30 text-amber-600 dark:text-amber-400',
                        ];
                    @endphp
                    <tr class="hover:bg-gray-50 dark:hover:bg-[#272B30] transition-colors" wire:key="post-{{ $post->id }}">
                        <td class="px-8 py-5">
                            <input
                        ...
                        <td class="px-4 py-5">
                            <div class="flex items-center gap-4">
                                ...
                                <img src="{{ asset('storage/' . $post->featured_image) }}" alt="" class="h-12 w-16 rounded-xl object-cover">
                                @else
                                <div class="h-12 w-16 rounded-xl bg-gray-100 dark:bg-[#272B30] flex items-center justify-center">
                                    <span class="material-symbols-outlined text-[#6F767E]">image</span>
                                </div>
                                @endif
                                <div>
                                    <p class="text-[15px] font-bold text-[#111827] dark:text-[#FCFCFC] line-clamp-1">{{ $post->title }}</p>
                                    <p class="text-xs text-[#6F767E] line-clamp-1">{{ Str::limit($post->excerpt, 50) }}</p>
                                </div>
                            </div>
                        </td>
                        <td class="px-4 py-5">
                            <div class="flex items-center gap-2">
                                ...
                                <span class="text-sm font-medium text-[#111827] dark:text-[#FCFCFC]">{{ $post->author->name ?? 'Unknown' }}</span>
                            </div>
                        </td>
                        <td class="px-4 py-5">
                            @if($post->categories->isNotEmpty())
                                <div class="flex flex-wrap gap-1">
                                    @foreach($post->categories->take(2) as $category)
                                        <span class="inline-flex items-center rounded-lg bg-gray-100 dark:bg-[#272B30] px-2 py-1 text-[10px] font-bold text-[#6F767E] dark:text-[#FCFCFC] uppercase tracking-wider">{{ $category->name }}</span>
                                    @endforeach
                                    @if($post->categories->count() > 2)
                                        <span class="inline-flex items-center rounded-lg bg-gray-100 dark:bg-[#272B30] px-2 py-1 text-[10px] font-bold text-[#6F767E]">+{{ $post->categories->count() - 2 }}</span>
                                    @endif
                                </div>
                            @else
                        ...
                        <td class="px-4 py-5">
                            <span class="inline-flex items-center gap-1.5 rounded-lg {{ $statusClasses[$post->status] ?? $statusClasses['draft'] }} px-2.5 py-1 text-[11px] font-bold uppercase tracking-wider">
                                <span class="h-1.5 w-1.5 rounded-full bg-current"></span>
                                {{ ucfirst($post->status) }}
                            </span>
                        </td>
                        <td class="px-4 py-5">
                            <p class="text-sm font-medium text-[#111827] dark:text-[#FCFCFC]">{{ number_format($post->views_count) }}</p>
                        </td>
                        <td class="px-4 py-5">
                            <p class="text-sm font-medium text-[#111827] dark:text-[#FCFCFC]">{{ $post->created_at->format('M d, Y') }}</p>
                            <p class="text-xs text-[#6F767E]">{{ $post->created_at->format('H:i') }}</p>
                        </td>
                        <td class="px-8 py-5 text-right">
                            <div class="flex gap-2 items-center justify-end">
                                @if($statusFilter === 'trashed')
                                    @can('posts.posts.delete')
                                    <button 
                                    ...
                                    <button 
                                        x-data
                                        @click="$dispatch('open-force-delete-modal', { postId: {{ $post->id }}, postTitle: '{{ addslashes($post->title) }}' })"
                                    ...
                                    @endcan
                                @else
                                    @can('posts.posts.view')
                                    <a href="{{ url($archiveSlug . '/' . $post->slug) }}" target="_blank"
                                    ...
                                    <button 
                                        x-data
                                        @click="$dispatch('open-delete-modal', { postId: {{ $post->id }}, postTitle: '{{ addslashes($post->title) }}' })"
                                        class="w-9 h-9 p-2 rounded-xl text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-950/20 flex items-center justify-center transition-colors"
                        ...
                        <td colspan="8" class="px-8 py-16 text-center">
                            <div class="flex flex-col items-center">
                                <div class="h-16 w-16 rounded-full bg-gray-100 dark:bg-[#272B30] flex items-center justify-center mb-4">
                                    <span class="material-symbols-outlined text-3xl text-[#6F767E]">article</span>
                                </div>
    ...
    <div class="fixed bottom-8 left-1/2 -translate-x-1/2 z-50">
        <div class="bg-[#2563EB] border border-[#2563EB] rounded-2xl shadow-2xl px-6 py-3 flex items-center gap-6">
            <div class="flex items-center gap-3 border-r border-blue-400/30 pr-6">
                <span class="bg-white text-[#2563EB] text-xs font-bold px-2.5 py-1 rounded-full min-w-[24px] text-center">{{ count($selectedPosts) }}</span>
    ...
    <!-- Delete Modal -->
    <div 
        x-data="{ show: false, postId: null, postTitle: '', bulk: false }"
        @open-delete-modal.window="show = true; postId = $event.detail.postId; postTitle = $event.detail.postTitle; bulk = false"
        ...
        <div 
            ...
            <div class="flex flex-col items-center text-center">
                ...
                <h3 class="text-2xl font-bold text-gray-900 dark:text-[#FCFCFC] mb-3">Delete Post</h3>
                <p class="text-gray-500 dark:text-[#6F767E] leading-relaxed mb-8">
                    <template x-if="bulk">
                        <span>Are you sure you want to delete <span class="font-bold">{{ count($selectedPosts) }}</span> selected post(s)?</span>
                    </template>
                    <template x-if="!bulk">
                        <span>Are you sure you want to delete "<span class="font-bold" x-text="postTitle"></span>"?</span>
                    </template>
                </p>
                <div class="flex items-center gap-3 w-full">
                    ...
                    <button 
                        @click="if (bulk) { $wire.deleteSelected(); } else { $wire.deletePost(postId); } show = false;"
    ...
    <div 
        ...
        <div 
            ...
            <div class="flex flex-col items-center text-center">
                ...
                <h3 class="text-2xl font-bold text-gray-900 dark:text-[#FCFCFC] mb-3">Publish Posts</h3>
                <p class="text-gray-500 dark:text-[#6F767E] leading-relaxed mb-8">
                    Are you sure you want to publish <span class="font-bold">{{ count($selectedPosts) }}</span> selected post(s)?
                </p>
                <div class="flex items-center gap-3 w-full">
                    <button @click="show = false"
                        class="flex-1 h-12 rounded-xl bg-gray-100 dark:bg-[#272B30] text-gray-700 dark:text-[#FCFCFC] text-sm font-bold hover:bg-gray-200 dark:hover:bg-[#33383f] transition-all">
    ...
    <div 
        ...
        <div 
            ...
            <div class="flex flex-col items-center text-center">
                ...
                <p class="text-gray-500 dark:text-[#6F767E] leading-relaxed mb-8">
                    Are you sure you want to move <span class="font-bold">{{ count($selectedPosts) }}</span> selected post(s) to draft?
                </p>
                <div class="flex items-center gap-3 w-full">
                    <button @click="show = false"
                        class="flex-1 h-12 rounded-xl bg-gray-100 dark:bg-[#272B30] text-gray-700 dark:text-[#FCFCFC] text-sm font-bold hover:bg-gray-200 dark:hover:bg-[#33383f] transition-all">
    ...
    <div 
        x-data="{ show: false, postId: null, postTitle: '', bulk: false }"
        @open-force-delete-modal.window="show = true; postId = $event.detail.postId; postTitle = $event.detail.postTitle; bulk = false"
        ...
        <div 
            ...
            <div class="flex flex-col items-center text-center">
                ...
                <p class="text-gray-500 dark:text-[#6F767E] leading-relaxed mb-8">
                    <template x-if="bulk">
                        <span>Are you sure you want to PERMANENTLY delete <span class="font-bold">{{ count($selectedPosts) }}</span> selected post(s)? This action cannot be undone.</span>
                    </template>
                    <template x-if="!bulk">
                        <span>Are you sure you want to PERMANENTLY delete "<span class="font-bold" x-text="postTitle"></span>"? This action cannot be undone.</span>
                    </template>
                </p>
                <div class="flex items-center gap-3 w-full">
                    <button @click="show = false"
                    ...
                    <button 
                        @click="if (bulk) { $wire.forceDeleteSelected(); } else { $wire.forceDelete(postId); } show = false;"
                        class="flex-1 h-12 rounded-xl bg-[#FF6A55] text-white text-sm font-bold hover:bg-[#E55F4D] transition-all shadow-lg shadow-[#FF6A55]/20">
                        Delete Permanently
                    </button>
                </div>
