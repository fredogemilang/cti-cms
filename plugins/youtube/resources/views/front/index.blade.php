...
@php
    ...
    $page = $page ?? null;
    $pageTitle = ($page && method_exists($page, 'getTranslation')) ? ($page->getTranslation('title', $locale, fallback: true) ?? $page->title) : t('nav.video', 'Video Library');

    // Featured video details initialization
    $initFeatured = $featured ?? null;
    $initId = $initFeatured ? $initFeatured->youtube_id : 'dQw4w9WgXcQ';
    $initTitle = $initFeatured ? $initFeatured->title : 'Transformasi Digital Enterprise: Roadmap Menuju Cloud-Native Architecture';
    $initCat = ($initFeatured && $initFeatured->playlists && $initFeatured->playlists->first()) 
        ? $initFeatured->playlists->first()->title 
        : ($initFeatured ? ($initFeatured->channel_title ?? 'Featured') : 'Webinar');
    $initDate = ($initFeatured && $initFeatured->published_at) ? $initFeatured->published_at->format('M d, Y') : 'May 15, 2025';
    $initAuthor = ($initFeatured && $initFeatured->channel_title) ? $initFeatured->channel_title : 'CDT Engineering';

    $activeCategory = $activeCategory ?? 'All';
    $searchQuery = $searchQuery ?? '';
@endphp

@section('title', $pageTitle . ' - ' . setting('site_name', 'Central Data Technology'))
...
<section class="pt-8 lg:pt-28 pb-20 bg-white relative overflow-hidden">
  <!-- Strong Red Gradient Orbs (bulat-bulat merah) -->
  <div
    class="absolute -top-10 left-0 md:left-1/4 w-[500px] h-[500px] bg-primary/20 rounded-full blur-[80px] pointer-events-none mix-blend-multiply">
  </div>
  <div
    class="absolute top-40 right-0 md:right-1/6 w-[600px] h-[600px] bg-red-500/15 rounded-full blur-[100px] pointer-events-none mix-blend-multiply">
  </div>
  <div
    class="absolute -bottom-20 left-1/3 w-[400px] h-[400px] bg-rose-500/10 rounded-full blur-[60px] pointer-events-none mix-blend-multiply">
  </div>
  <div class="mx-auto max-w-[1400px] px-4 sm:px-6 lg:px-8 relative z-10">
    <!-- Breadcrumb (SEO CMS) -->
    <x-seo-breadcrumbs :entity="$page ?? null" class="flex items-center space-x-2 text-xs font-semibold tracking-wide text-zinc-400 mb-10 text-left" />
    ...
    <div class="mx-auto max-w-[1200px] relative">
      ...
      <div class="overflow-hidden text-center mb-10">
        <h1 class="text-4xl md:text-5xl lg:text-[54px] font-bold text-gray-900 leading-tight" data-gsap="fade-up"
          style="font-family: var(--font-heading);">{{ $pageTitle }}</h1>
      </div>

      <!-- Main Player -->
      <div class="player-glow mb-6" data-gsap="fade-up" data-gsap-delay="0.1">
        <div class="relative w-full bg-black rounded-2xl overflow-hidden shadow-xl aspect-video z-10"
          id="player-aspect-wrap">
      ...
      <!-- Video Details -->
      <div id="details-container-col" class="mb-4" data-gsap="fade-up" data-gsap-delay="0.15">
        <h2 id="main-title" class="text-xl md:text-2xl font-bold text-gray-900 leading-snug"
          style="font-family: var(--font-heading);">{{ $initTitle }}</h2>
        <div id="main-meta-line" class="video-meta mt-3">
          <span id="main-category-badge"
            class="inline-block px-2.5 py-0.5 bg-red-50 border border-red-100 rounded-full text-[11px] font-bold text-primary uppercase tracking-wider">{{ $initCat }}</span>
          <span class="dot"></span>
          <span id="main-date" class="text-gray-400 text-xs">{{ $initDate }}</span>
          <span class="dot"></span>
          <span id="main-author" class="text-gray-400 text-xs">{{ $initAuthor }}</span>
        </div>
      </div>
    </div>
  </div>
...
<!-- All Videos Section -->
<section id="video-list" class="pb-20 bg-gray-50/60">
  <div class="mx-auto max-w-[1400px] px-4 sm:px-6 lg:px-8">

    <!-- Section Header + Filters -->
    <div class="pt-10 mb-8" data-gsap="fade-up" data-gsap-delay="0.2">
      <div class="flex flex-col md:flex-row md:items-center justify-between gap-5 mb-8">
        <span class="section-label">{{ t('common.all_videos', 'All Videos') }}</span>

        <!-- Search Form (Server-Side) -->
        <form method="GET" action="{{ request()->url() }}#video-list" class="relative w-full md:w-72">
          ...
          <input type="text" name="q" value="{{ $searchQuery }}" placeholder="{{ t('common.search_videos', 'Search videos...') }}" class="search-light">
          @if(!empty($searchQuery))
            <a href="{{ request()->fullUrlWithQuery(['q' => null, 'page' => 1]) }}#video-list"
              class="absolute inset-y-0 right-0 pr-3.5 flex items-center text-gray-400 hover:text-gray-600 transition-colors" title="Clear search">
      ...
      <!-- Category Pills (Server-Side Filter Links) -->
      <div class="flex items-center gap-2 overflow-x-auto scrollbar-none pt-0 pl-3 pr-3 pb-4">
        <a href="{{ request()->fullUrlWithQuery(['category' => 'All', 'page' => 1]) }}#video-list" 
           class="cat-pill {{ $activeCategory === 'All' ? 'active' : '' }}">
          {{ t('common.all', 'All') }}
        </a>
        @if(isset($playlists) && $playlists->isNotEmpty())
          @foreach($playlists as $pl)
            <a href="{{ request()->fullUrlWithQuery(['category' => $pl->title, 'page' => 1]) }}#video-list" 
               class="cat-pill {{ $activeCategory === $pl->title ? 'active' : '' }}">
              {{ $pl->title }}
            </a>
          @endforeach
        @else
          <a href="{{ request()->fullUrlWithQuery(['category' => 'Webinar', 'page' => 1]) }}#video-list" class="cat-pill {{ $activeCategory === 'Webinar' ? 'active' : '' }}">Webinar</a>
          <a href="{{ request()->fullUrlWithQuery(['category' => 'Security', 'page' => 1]) }}#video-list" class="cat-pill {{ $activeCategory === 'Security' ? 'active' : '' }}">Security</a>
          <a href="{{ request()->fullUrlWithQuery(['category' => 'Engineering', 'page' => 1]) }}#video-list" class="cat-pill {{ $activeCategory === 'Engineering' ? 'active' : '' }}">Engineering</a>
          <a href="{{ request()->fullUrlWithQuery(['category' => 'Events', 'page' => 1]) }}#video-list" class="cat-pill {{ $activeCategory === 'Events' ? 'active' : '' }}">Events</a>
          <a href="{{ request()->fullUrlWithQuery(['category' => 'Products', 'page' => 1]) }}#video-list" class="cat-pill {{ $activeCategory === 'Products' ? 'active' : '' }}">Products</a>
        @endif
      </div>
    </div>
    ...
    @if(isset($videos) && method_exists($videos, 'hasPages') && $videos->hasPages())
      <div class="mt-12 flex justify-center">
        {{ $videos->links('youtube::front.pagination') }}
      </div>
    @endif
  </div>
</section>
...
<script>
  const DB_VIDEO_DATA = @json($videoJsonData ?? []);
  ...
  const FALLBACK_VIDEO_DATA = [
    {
      ...
      avatar: "CDT",
      thumbnail: "/assets/images/youtube/dQw4w9WgXcQ-mqdefault.jpg"
    },
    {
      id: "ScMzIvxBSi4",
      ...
      company: "Central Data Technology",
      avatar: "SEC",
      thumbnail: "/assets/images/youtube/ScMzIvxBSi4-mqdefault.jpg"
    },
    {
      id: "9bZkp7q19f0",
      ...
      company: "Central Data Technology",
      avatar: "CDT",
      thumbnail: "/assets/images/youtube/9bZkp7q19f0-mqdefault.jpg"
    },
    {
      id: "jNQXAC9IVRw",
      ...
      company: "Central Data Technology",
      avatar: "EVE",
      thumbnail: "/assets/images/youtube/jNQXAC9IVRw-mqdefault.jpg"
    },
    {
      id: "LXb3EKWsInQ",
      ...
      company: "Central Data Technology",
      avatar: "PRO",
      thumbnail: "/assets/images/youtube/LXb3EKWsInQ-mqdefault.jpg"
    },
    {
      id: "aircAruvnKk",
      ...
      company: "Central Data Technology",
      avatar: "CDT",
      thumbnail: "/assets/images/youtube/aircAruvnKk-mqdefault.jpg"
    },
    {
      id: "YQHsXMglC9A",
      ...
      company: "Central Data Technology",
      avatar: "SEC",
      thumbnail: "/assets/images/youtube/YQHsXMglC9A-mqdefault.jpg"
    },
    {
      id: "aqz-KE-bpKQ",
      ...
      company: "Central Data Technology",
      avatar: "PRO",
      thumbnail: "/assets/images/youtube/aqz-KE-bpKQ-mqdefault.jpg"
    }
  ];

  const VIDEO_DATA = (DB_VIDEO_DATA && DB_VIDEO_DATA.length > 0) ? DB_VIDEO_DATA : FALLBACK_VIDEO_DATA;

  // App States
  let currentVideoId = @json($initId);
  let ytPlayer = null;
  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    // Auto smooth scroll to video list section when filtering category, searching, or paginating
    if (window.location.hash === '#video-list' || urlParams.has('category') || urlParams.has('page') || urlParams.has('q')) {
      const targetSection = document.getElementById('video-list');
      if (targetSection) {
        const headerOffset = 100;
        const rect = targetSection.getBoundingClientRect();
        const scrollTarget = rect.top + window.pageYOffset - headerOffset;
        setTimeout(function() {
  ...
  function generateCardHTML(video, isActive) {
    ...
    const nowPlaying = isActive
      ...
      : '';
    const thumbUrl = video.thumbnail || `/assets/images/youtube/${video.id}-mqdefault.jpg`;

    return `
    <a href="#" class="video-card ${activeClass} video-item flex flex-col" data-video="${video.id}">
      <div class="card-thumb relative w-full aspect-video">
