    ...
    <div class="prod-detail-grid">
      ...
      <div class="prod-detail-gallery">
        ...
        <div class="gallery-main-wrapper">
          ...
          <x-image :src="$entry->featured_image ?: $mainImg" alt="{{ $entry->getTranslation('title') }}" class="gallery-main-img" sizes="100vw" id="main-prod-image" @if(str_contains($entry->slug, 'gateway')) style="max-width: 320px !important; height: auto !important;" @endif />
        </div>
          ...
          <div class="gallery-thumbs" id="prod-thumbs">
            @foreach($galleryImages as $index => $imgUrl)
              <div class="gallery-thumb-item {{ $index === 0 ? 'active' : '' }}" onclick="changeImage('{{ $imgUrl }}', this)">
                <x-image :src="$imgUrl" alt="View {{ $index + 1 }}" class="w-full h-full object-contain p-1" sizes="100vw" />
              </div>
            @endforeach
          </div>
      ...
      @foreach($applications as $app)
        ...
        <a href="{{ $url }}" class="app-card">
          <div class="app-card-img-wrapper">
            <x-image :src="$app['image']" alt="{{ $app['title'] ?? '' }}" class="w-full h-full object-cover" sizes="100vw" />
          </div>
