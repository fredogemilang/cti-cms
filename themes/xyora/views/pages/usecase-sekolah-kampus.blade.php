...
@section('title', $page->title ?? 'XYORA - Sekolah & Kampus')

@section('content')
@php
  $heroBg = $page->block('hero_bg', 'images/sekolah-kampus.png');
  $heroBgUrl = resolve_block_asset($heroBg);
  
  $aboutImage = $page->block('about_image', 'images/about-sekolah-kampus.png');
...
<h1 class="sr-only">
  {{ t('usecase.school_h1_seo', 'XYORA - Solusi Jaringan Wi-Fi Premium untuk Sekolah & Kampus') }}
</h1>
    ...
    <div class="usecase-content-container">
      <div class="usecase-challenge-card" style="background-image: linear-gradient(90deg, #ffffff 0%, #ffffff 55%, rgba(255, 255, 255, 0) 100%), url('{{ $aboutImageUrl }}');">
        ...
        <div class="usecase-challenge-info">
          ...
          <p>
            {{ t('usecase.school_challenge_p1', 'Sekolah dan kampus saat ini membutuhkan jaringan yang mampu mendukung ribuan pengguna dan perangkat yang terhubung secara bersamaan untuk berbagai aktivitas digital, mulai dari Learning Management System (LMS), video conference, akses cloud, hingga layanan akademik berbasis online. Namun, banyak kampus dan sekolah masih mengandalkan infrastruktur jaringan yang belum dirancang untuk menangani kebutuhan bandwidth tinggi dan kepadatan pengguna yang terus meningkat.') }}
          </p>
          <p>
            {{ t('usecase.school_challenge_p2', 'Akibatnya, koneksi sering melambat saat jam sibuk, muncul area dengan sinyal lemah, roaming antar gedung tidak berjalan mulus, serta risiko keamanan meningkat karena perangkat jaringan yang sudah usang. Kondisi ini dapat mengganggu pengalaman belajar, produktivitas staf, dan efektivitas operasional institusi secara keseluruhan.') }}
          </p>
        </div>

        <!-- Right: Image -->
        <div class="usecase-challenge-image">
          <img src="{{ $aboutImageUrl }}" alt="Tantangan Jaringan Sekolah & Kampus" />
        </div>
      </div>
    </div>
  </section>
