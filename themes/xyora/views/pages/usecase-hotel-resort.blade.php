...
@section('title', $page->title ?? 'XYORA - Hotel & Resort')

@section('content')
@php
  $heroBg = $page->block('hero_bg', 'images/hotel-resort.png');
  $heroBgUrl = resolve_block_asset($heroBg);
  
  $aboutImage = $page->block('about_image', 'images/about-hotel-resort.png');
...
<h1 class="sr-only">
  {{ t('usecase.hotel_h1_seo', 'XYORA - Solusi Jaringan Wi-Fi Premium untuk Hotel & Resort') }}
</h1>
    ...
    <div class="usecase-content-container">
      <div class="usecase-challenge-card" style="background-image: linear-gradient(90deg, #ffffff 0%, #ffffff 55%, rgba(255, 255, 255, 0) 100%), url('{{ $aboutImageUrl }}');">
        ...
        <div class="usecase-challenge-info">
          ...
          <p>
            {{ t('usecase.hotel_challenge_p1', 'Kebutuhan konektivitas di lingkungan hotel terus meningkat seiring penggunaan berbagai aplikasi digital, layanan streaming, video conference, sistem operasional berbasis cloud, hingga akses internet tamu yang semakin tinggi. Namun, banyak infrastruktur WiFi masih menghadapi keterbatasan kapasitas sehingga access point mudah mengalami overload saat menangani banyak perangkat secara bersamaan. Kondisi ini sering menyebabkan koneksi melambat, buffering, latensi tinggi, hingga pengalaman pengguna yang tidak konsisten, terutama pada area dengan tingkat okupansi tinggi.') }}
          </p>
          <p>
            {{ t('usecase.hotel_challenge_p2', 'Di sisi lain, karakteristik bangunan hotel yang terdiri dari banyak ruangan, dinding tebal, dan area bertingkat sering menimbulkan dead spots serta kualitas sinyal yang tidak merata. Penempatan access point yang kurang optimal juga dapat memicu interferensi antar perangkat sehingga performa jaringan semakin menurun. Selain berdampak pada kenyamanan tamu dan produktivitas staf, infrastruktur yang belum didukung sistem keamanan modern turut meningkatkan risiko akses tidak sah dan kebocoran data yang dapat memengaruhi operasional maupun reputasi hotel.') }}
          </p>
        </div>

        <!-- Right: Image -->
        <div class="usecase-challenge-image">
          <img src="{{ $aboutImageUrl }}" alt="Tantangan Jaringan Hotel & Resort" />
        </div>
      </div>
    </div>
  </section>
      ...
      @foreach($solusiCards as $card)
        <div class="solusi-card">
          <div class="solusi-badge">
            <img src="{{ resolve_block_asset($card['badge'] ?? 'icons/verified.png') }}" alt="Verified Badge" />
          </div>
          <div class="solusi-card-image">
            <img src="{{ resolve_block_asset($card['image'] ?? 'images/solusi1.png') }}" alt="{{ $card['title'] ?? 'Solusi' }}" />
          </div>
    ...
    <div class="manajemen-container">
      <div class="manajemen-card" style="background: linear-gradient(270deg, rgba(255, 255, 255, 0) 0%, #E0EEFB 55.92%), url('{{ $manajemenImageUrl }}') no-repeat right center / cover;">
        ...
        <div class="manajemen-image">
          <img src="{{ $manajemenImageUrl }}" alt="Manajemen Jaringan Terpusat Xyora" />
        </div>
      </div>
    </div>
  </section>
