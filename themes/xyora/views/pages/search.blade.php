...
<main class="search-page-main" style="background: #ffffff; padding-top: 60px; padding-bottom: 80px;">
  <div class="search-page-container" style="max-width: 1200px; margin: 0 auto; padding: 0 20px;">
    ...
    <div class="search-page-header" style="margin-bottom: 50px; text-align: center;">
      <h2 style="font-family: 'Outfit', sans-serif; font-size: 32px; font-weight: 700; color: #111e38; margin-bottom: 15px;">
      ...
      <form action="{{ app()->getLocale() !== setting('default_locale', 'en') ? url(app()->getLocale() . '/search') : url('/search') }}" method="GET" style="max-width: 600px; margin: 0 auto; display: flex; gap: 10px; box-shadow: 0 4px 20px rgba(17, 30, 56, 0.08); padding: 6px; border-radius: 50px; border: 1px solid #e1e8f0; background: #fff;">
        <input type="search" name="q" value="{{ $query }}" placeholder="{{ t('search.placeholder', 'Cari produk atau artikel...') }}" required style="flex: 1; border: none; outline: none; padding: 12px 24px; font-size: 15px; border-radius: 50px; font-family: 'Inter', sans-serif;" />
        <button type="submit" style="background: #89C55C; color: #fff; border: none; outline: none; padding: 12px 30px; font-size: 15px; font-weight: 600; border-radius: 50px; cursor: pointer; transition: background 0.2s ease; font-family: 'Outfit', sans-serif;">
    ...
    @if(empty($query))
      <div style="text-align: center; padding: 60px 20px; color: #999; font-size: 18px; font-family: 'Inter', sans-serif;">
    ...
    @elseif($blogResults->isEmpty() && $productResults->isEmpty())
      <div style="text-align: center; padding: 80px 20px; color: #666; font-family: 'Inter', sans-serif;">
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="#ccc" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" width="80" height="80" style="margin-bottom: 20px;">
          <circle cx="11" cy="11" r="8" />
          <path d="m21 21-4.3-4.3" />
        </svg>
        <h3 style="font-family: 'Outfit', sans-serif; font-size: 22px; font-weight: 600; color: #111e38; margin-bottom: 10px;">
    ...
    @else
      ...
      @if(!$productResults->isEmpty())
        <section class="search-product-results" style="margin-bottom: 70px;">
          <div style="border-bottom: 2px solid #f1f5f9; padding-bottom: 15px; margin-bottom: 30px; display: flex; align-items: center; gap: 10px;">
            <span style="background: rgba(137, 197, 92, 0.1); color: #89C55C; width: 36px; height: 36px; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 18px; font-weight: bold;">
              🔌
            </span>
            <h3 style="font-family: 'Outfit', sans-serif; font-size: 24px; font-weight: 700; color: #111e38; margin: 0;">
              ...
              <span style="font-size: 16px; font-weight: 500; color: #888; margin-left: 8px;">({{ $productResults->count() }})</span>
            </h3>
          </div>

          <div class="products-grid" style="display: grid; grid-template-columns: repeat(auto-fill, minmax(220px, 1fr)); gap: 30px;">
      ...
      @if(!$blogResults->isEmpty())
        <section class="search-blog-results">
          <div style="border-bottom: 2px solid #f1f5f9; padding-bottom: 15px; margin-bottom: 30px; display: flex; align-items: center; gap: 10px;">
            ...
            <h3 style="font-family: 'Outfit', sans-serif; font-size: 24px; font-weight: 700; color: #111e38; margin: 0;">
              {{ t('search.blog_section_title', 'Artikel & Edukasi') }} 
              <span style="font-size: 16px; font-weight: 500; color: #888; margin-left: 8px;">({{ $blogResults->count() }})</span>
            </h3>
          </div>
