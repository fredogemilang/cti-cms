              ...
              <div class="expertise-card bg-white shadow-sm border border-zinc-100 overflow-hidden hover:shadow-md transition-shadow rounded-2xl flex flex-col">
                @if($itemImgUrl)
                  <div class="h-40 w-full overflow-hidden shrink-0">
                    <x-image :src="$itemImgUrl" alt="{{ $item['title'] ?? 'Expertise' }}" title="{{ $item['title'] ?? 'Expertise' }}" sizes="(max-width: 768px) 100vw, (max-width: 1200px) 33vw, 380px" class="w-full h-full object-cover" />
                  </div>
                @endif
            ...
            @foreach(($partners ?? []) as $partner)
              ...
              @if($logoUrl)
                <a href="{{ $partner->getUrl() }}" x-link
                  ...
                  <x-image :src="$logoUrl" alt="{{ $partner->title }}" title="{{ $partner->title }}" sizes="(max-width: 768px) 50vw, 200px" class="alliance-logo w-full h-full object-contain" />
                </a>
              @endif
            @endforeach
          </div>
          ...
          <div class="grid grid-cols-1 md:grid-cols-3">
            @foreach($whyCdtItems as $index => $item)
              ...
              <div class="relative h-[320px] group overflow-hidden" data-gsap="curtain-reveal" data-gsap-delay="{{ $delay }}">
                <x-image :src="$bgImgUrl" alt="{!! strip_tags($item['title'] ?? 'Why CDT') !!}" title="{!! strip_tags($item['title'] ?? 'Why CDT') !!}" sizes="(max-width: 768px) 100vw, 25vw" class="absolute inset-0 w-full h-full object-cover grayscale group-hover:scale-105 group-hover:blur-[3px] transition-all duration-700" />
