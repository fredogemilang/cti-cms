...
@section('content')
    ...
    <section class="booking-section py-5 position-relative">
        <div class="custom-shape-divider-top-booking"></div>
        <div class="container py-5">
            ...
            <div class="card p-4 p-lg-5 rounded-4 border-0 shadow-lg mx-auto" style="max-width: 900px;" data-aos="fade-up" data-aos-delay="100">
                ...
                @if($event->is_registration_open)
                    {{-- Livewire registration form (PRD 02) --}}
                    <livewire:plugins.event-registration-form :slug="$event->slug" />
                @else
                    ...
                    <div class="text-center py-5">
                        @if($event->max_participants && $event->registered_count >= $event->max_participants)
                            <svg xmlns="http://www.w3.org/2000/svg" width="56" height="56" fill="none" viewBox="0 0 24 24"
                                stroke="currentColor" stroke-width="1.5" class="text-danger mb-3">
                                <path stroke-linecap="round" stroke-linejoin="round"
                                    d="M18.364 18.364A9 9 0 005.636 5.636m12.728 12.728A9 9 0 015.636 5.636m12.728 12.728L5.636 5.636" />
                            </svg>
                            <h5 class="fw-bold text-danger mb-2">Event is Full</h5>
                            <p class="text-muted">This event has reached its maximum capacity.</p>
                        @elseif($event->registration_end_date && $event->registration_end_date->isPast())
                            <svg xmlns="http://www.w3.org/2000/svg" width="56" height="56" fill="none" viewBox="0 0 24 24"
                                stroke="currentColor" stroke-width="1.5" class="text-secondary mb-3">
                                <path stroke-linecap="round" stroke-linejoin="round"
                                    d="M12 6v6h4.5m4.5 0a9 9 0 11-18 0 9 9 0 0118 0z" />
                            </svg>
                            <h5 class="fw-bold mb-2">Registration Closed</h5>
                            <p class="text-muted">The registration period for this event has ended.</p>
                        @elseif($event->registration_start_date && $event->registration_start_date->isFuture())
                            <svg xmlns="http://www.w3.org/2000/svg" width="56" height="56" fill="none" viewBox="0 0 24 24"
                                stroke="currentColor" stroke-width="1.5" class="text-warning mb-3">
                                <path stroke-linecap="round" stroke-linejoin="round"
                                    d="M6.75 3v2.25M17.25 3v2.25M3 18.75V7.5a2.25 2.25 0 012.25-2.25h13.5A2.25 2.25 0 0121 7.5v11.25" />
                            </svg>
                            <h5 class="fw-bold mb-2">Registration Opens Soon</h5>
                            <p class="text-muted">
                                Registration opens on {{ $event->registration_start_date->isoFormat('D MMMM YYYY') }}.
                            </p>
                        @else
                            <h5 class="fw-bold mb-2">Registration Not Available</h5>
                            <p class="text-muted">Registration for this event is currently unavailable.</p>
                        @endif
                    </div>
                @endif
